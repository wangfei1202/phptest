...
class OwmImagesService{

	// 返回GD函数版本号
	public static function Ver(){
		if (function_exists('gd_info')){
			$gdArr = gd_info();
			$gdVer = $gdArr['GD Version'] ? $gdArr['GD Version'] : 0;
			unset($gdArr);
		} else {
			$gdVer = 0;
		}
	...
	*/
	public static function Watermark($wmArr=array()){
		$upLoadImg		= isset($wmArr['upLoadImg']) && empty($wmArr['upLoadImg'])==false ? Str::RelToAbs($wmArr['upLoadImg']) : NULL;
		$waterImg		= isset($wmArr['waterImg']) && empty($wmArr['waterImg'])==false ? Str::RelToAbs($wmArr['waterImg']) : '';
			/* if (strlen($waterImg) < 3){
				die('您开启了水印功能，但没上传水印图，请先到【常规设置】-【图片生成设置】关闭水印功能或者上传水印图。');
			} */
		$waterPos		= isset($wmArr['waterPos']) && empty($wmArr['waterPos'])==false ? $wmArr['waterPos'] : 'rightBottom';
		$waterPadding	= isset($wmArr['waterPadding']) && empty($wmArr['waterPadding'])==false ? $wmArr['waterPadding'] : 3;
		$waterTrans		= isset($wmArr['waterTrans']) && empty($wmArr['waterTrans'])==false ? $wmArr['waterTrans'] : 80;
			if ($waterTrans < 10){ $waterTrans = 80; }
		$waterText		= isset($wmArr['waterText']) && empty($wmArr['waterText'])==false ? $wmArr['waterText'] : '';
		$textFont		= isset($wmArr['textFont']) && empty($wmArr['textFont'])==false ? $wmArr['textFont'] : 5;
		$textColor		= isset($wmArr['textColor']) && empty($wmArr['textColor'])==false ? $wmArr['textColor'] : '#FF0000';
		$upLoadExt = strtolower(pathinfo($upLoadImg, PATHINFO_EXTENSION));
		$waterExt = strtolower(pathinfo($waterImg, PATHINFO_EXTENSION));

		if ($upLoadExt == 'gif' && $waterExt == 'png'){
		...
		if ((! empty($waterImg)) && file_exists($waterImg)){
			...
			$water_im = self::CreateFrom($waterImg, $water_info[2]);
				if ($water_im == null){
					die('暂不支持该水印图文件格式（'. $water_info[2] .'），请用图片处理软件将图片转换为GIF、JPG、PNG格式。');
				}
		}else{
			$fontFile = OT::GetFontAbsPath();
			if (strlen($fontFile) == 0){ $fontFile = OT_ROOT .'inc/imageWatermark.ttf'; }
		...
		if (! empty($upLoadImg) && file_exists($upLoadImg)){ 
			...
			$ground_im = self::CreateFrom($upLoadImg, $ground_info[2]);
				if ($ground_im == null){
					return '';
					// die('暂不支持该图片文件格式（'. $ground_info[2] .'），请用图片处理软件将图片转换为GIF、JPG、PNG格式。')
				}
		} else {
			die('需要加水印的图片不存在！');
		}
	...
	*/
	...
	//		if ( $imgInfo[0] > $thumbWidth || $imgInfo[1] > $thumbHeight ||  ){	//原图片的宽或高小于缩略图宽或高,则不生成缩略图
				$thumbArr = self::ThumbSize( array(
					'thumbWidth'  => $thumbWidth,
					'thumbHeight' => $thumbHeight,
					'imgWidth'    => $imgInfo[0],
					'imgHeight'   => $imgInfo[1]
				));
				$return['width']   = $thumbArr['newWidth'];
				$return['height']  = $thumbArr['newHeight'];
				if ( $imgExt == 'gif' ){
				...
				if ( $image ){
					if (function_exists('imagecopyresampled')){
						$thumb = imagecreatetruecolor($thumbArr['newWidth'], $thumbArr['newHeight']);
						if ($imgExt == 'png'){
						...
						imagecopyresampled($thumb, $image, 0, 0, 0, 0, $thumbArr['newWidth'], $thumbArr['newHeight'], $imgInfo[0], $imgInfo[1]);
					} else {
						$thumb = imagecreate($thumbArr['newWidth'], $thumbArr['newHeight']);
						if ($imgExt == 'png'){
							$color=imagecolorallocate($thumb,255,255,255); 
							imagecolortransparent($thumb,$color); 
							imagefill($thumb,0,0,$color); 
						...
						imagecopyresized($thumb, $image, 0, 0, 0, 0, $thumbArr['newWidth'], $thumbArr['newHeight'], $imgInfo[0], $imgInfo[1]);
					}
	...
	*/
	...
	// 计算缩略图的大小
	public static function ThumbSize($imgArr) {
		if (intval($imgArr['thumbWidth'])==0 && intval($imgArr['thumbHeight'])==0){
			$imgArr['newWidth']	= 100;
			$imgArr['newHeight']	= 100;
		} elseif ( intval($imgArr['thumbHeight'])==0 ) {
			$imgArr['newWidth']	= $imgArr['thumbWidth'];
			$imgArr['newHeight']	= ceil( $imgArr['imgHeight'] * $imgArr['thumbWidth'] / $imgArr['imgWidth'] );
		} elseif ( intval($imgArr['thumbWidth'])==0 ) {
			$imgArr['newWidth']	= ceil( $imgArr['imgWidth'] * $imgArr['thumbHeight'] / $imgArr['imgHeight'] );
			$imgArr['newHeight']	= $imgArr['thumbHeight'];
		} else {
			$imgArr['newWidth']	= $imgArr['thumbWidth'];
			$imgArr['newHeight']	= $imgArr['thumbHeight'];
		}
	...
	public static function UnsharpMask($img, $amount = 100, $radius = .5, $threshold = 3){
		...
		for ($i = 0; $i < $radius; $i++){
			...
			ImageCopyMerge($imgBlur, $imgCanvas, 0, 1, 0, 0, $w, $h, 16); // dow
			...
			ImageCopyMerge($imgBlur2, $imgCanvas2, 0, 0, 0, 0, $w, $h, 16);
			ImageCopyMerge($imgBlur2, $imgCanvas2, 0, 0, 0, 0, $w, $h, 50);
	...
	*/
	...
	// getimagesize获取的图片类型转为字符串信息
	public static function NumToType($num){
		return image_type_to_extension($num, false);
		/*
		switch ($num){
			...
			case 5:		return 'psd';
			case 6:		return 'bmp';
			case 7:		return 'tiff_ii';	// TIFF(intel byte order)
			case 8:		return 'tiff_mm';	// TIFF(motorola byte order)
			case 9:		return 'jpc';
			case 10:	return 'jp2';
			case 11:	return 'jpx';
			case 12:	return 'jb2';
	...
	// imagecreatefrom 系列函数，从文件或 URL 载入一幅图像，成功返回图像资源，失败则返回一个空字符串
	// $imgPath 图片文件路径； $type 类型可以是getimagesize[2]数值或者jpg、png； $typeArr 允许检测类型jpg、png
	public static function CreateFrom($imgPath, $type='', $typeArr = array()){
		if (strlen($type) > 0){
			if (is_numeric($type)){
				// $type = self::NumToType($type);
			}else{
				$type = strtolower($type);
			}
		}else{
			$type = strtolower(pathinfo($imgPath, PATHINFO_EXTENSION));
		}
		switch($type){	//取得背景图片的格式
			case 1: case 'gif':
				// if (count($typeArr) > 0 && in_array('gif',$typeArr) == false){
				//	$ret = null;
				// }else{
			...
			/* 
			case 6: case 'bmp':
				$ret = imagecreatefrombmp($imgPath);
				break;
	...
	// 判断远程文件是否是有效图片
	public static function JudRemote($fileUrl){
		$stream_opts = array(
			'ssl' => array(
				'verify_host' => false,
				'verify_peer' => false,
		...
		$ext = '';
		// 获取响应头
		if(( $headers = get_headers($fileUrl, 1, stream_context_create($stream_opts)) ) !== false){
			// 获取响应的类型
